...
document
    ...
    .addEventListener('click', searchDonors);

document
    .getElementById('s-city')
...
async function searchDonors() {
    ...
    const res = await fetch(
        `/api/search?blood_group=${encodeURIComponent(bg)}&city=${encodeURIComponent(city)}`
    );
        ...
        <div class="donor-card ${d.available ? '' : 'unavailable'}">
            ...
            ${
                d.available
                    ...
                    <button
                        class="mark-btn"
                        onclick="markDonation('${escHtml(d.name)}','${escHtml(d.phone)}')">
...
function renderEmergencyResults(donors, bg, city) {
    ...
    if (!donors.length) {
        ...
        area.innerHTML = `
            <div class="em-match-header not-found">
                ...
                No available ${escHtml(bg)} donors found in
                "${escHtml(city || 'any city')}".

                Please try broadening your search or
    ...
    area.innerHTML = `
        ...
        <div class="em-match-header found">

            ✔ Found ${donors.length}
            matching donor${donors.length > 1 ? 's' : ''}
            for blood group ${escHtml(bg)}
    ...
    showToast(
        `Found ${donors.length} donor(s)! Contact them immediately.`,
        'success'
    );
        ...
        return `
            ...
            <tr>
                ...
                <td>
                    ...
                    <span class="type-tag ${isDonation ? 'donation' : 'emergency'}">

                        ${isDonation ? '✔ Donation' : '🚨 Emergency'}
